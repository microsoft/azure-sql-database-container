...
> The image is x64 (`linux/amd64`); on a non-x64 host, add `--platform linux/amd64`. `CREATE VECTOR INDEX` (DiskANN) is still in development, so use full-scan similarity for now.
