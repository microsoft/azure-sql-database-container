...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Container exits right after start; logs say `requires at least 2000 megabytes of memory` | Engine has less than 2 GB | Raise the memory available to your container engine (the engine needs at least 2 GB). |
...
| `docker pull` fails with auth error | Not signed in to the private registry | `docker login sqldbpreview-dpgaeqhmgphzd4bk.azurecr.io -u <username>` with the welcome-email credentials. |
...
| Image will not pull or run (`no matching manifest`) | The image is x64 (`linux/amd64`) | On a non-x64 host, add `--platform linux/amd64`. |
