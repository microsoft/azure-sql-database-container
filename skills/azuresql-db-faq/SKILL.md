...
- **"Is Microsoft Entra ID (Azure AD) authentication supported?"** No, not on the container today. Use SQL authentication locally (the `sa` login, or a contained user). The SQL Server image (`mcr.microsoft.com/mssql/server`) does support it via `MSSQL_AAD_CLIENT_ID` / `MSSQL_AAD_PRIMARY_TENANT` / `MSSQL_AAD_CERTIFICATE_FILE_PATH`, which is why people expect it here; **those variables have no effect on this image** and setting them will not enable Entra. This does not break local-to-cloud: Azure SQL Database in the cloud does support Entra, so the normal pattern is SQL auth locally and Entra in the cloud, changing only the connection string (see the `azuresql-db-local-to-cloud` skill). Do not promise a date; say it is not supported today and point at the live Known limitations page.
