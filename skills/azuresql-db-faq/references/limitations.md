...
4. **x64 image only; no native ARM64 build.** The image is `linux/amd64`. On an ARM64 host (Apple Silicon, ARM64 Windows/Linux) it runs under emulation: add `--platform linux/amd64` (Docker) or `platform: linux/amd64` (compose). Emulation costs startup and query performance. Never say ARM64 is "supported". A native build is not available today and has no announced date; feature requests: https://aka.ms/azuresql-developer-feature-request
